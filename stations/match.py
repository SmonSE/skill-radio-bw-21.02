...
def match_station_name(phrase, station, aliases, radio_keyword):
    ...
    Args:
        ...
        radio_keyword (str): localized keyword for "radio"
    ...
    # If phrase contains both a station acronym and the radio keyword
    # ensure minimum confidences.
    if f"{station.acronym.lower()} {radio_keyword}" in phrase:
        # Eg "play DLF radio"
        match_confidences.append(CONF_HIGH_MATCH)
    elif station.acronym.lower() in phrase:
        if radio_keyword in phrase:
            # Eg "what's the radio on DLF"
...
def match_station_from_utterance(skill, utterance):
    ...
    Returns:
        Station or None if radio not requested.
    """
    ...
    # If radio vocab does exist, provide a minimum default confidence
    default_station = skill.get_default_station()
    match = Match(default_station, CONF_GENERIC_MATCH)

    # Catch any short explicit phrases eg 'play the radio'
    radio_phrases = skill.translate_list("PlayTheRadio") or []
    if utterance in radio_phrases:
    ...
    radio_keyword = skill.translate('OnlyRadio').lower()
    LOG.debug("Matching against specific stations")
    for station in stations.values():
        aliases = skill.alternate_station_names.get(station.acronym)
        station_match = match_station_name(utterance, station, aliases, radio_keyword)
